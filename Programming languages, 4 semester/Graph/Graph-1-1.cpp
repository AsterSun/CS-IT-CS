...
public:
    ...
    int countOfAdjVertex(int vertex)
    {                               // Количество смежных вершин
        auto it = Adj.find(vertex); // Ищем нужную вершину
        int result = 0;             // Инициализиуруем переменную для результата
        for (auto it_1 = it->second.begin(); it_1 != it->second.end(); it_1++)
            result++;  // Пока есть смежные вершины - увеличиваем результат
...
{   // Ввод базовых параметров графа
    ...
    // Вывод числа смежных вершин с нужной. Задание дано для неориентированного графа,
    // для ориентированного графа будет выведена степень полуисхода, так как для x
    // считаются только рёбра x->y, но не y->x
    cout << endl << "Enter vertex:" << endl;
    int vr;
    cin >> vr;
    cout << "Count of adjacent vertexes: " << gr.countOfAdjVertex(vr) << endl;
