...
public:
    ...
    // Функция добавления ребра. Аналогична созданию списка смежности,
    // но в данном случае нам не нужно циклом добавлять каждое ребро,
    // а только одно. Ищем вершину и уже к ней добавляем ребро.
    // Функция работает как для ориентированного, так и для
    ...
    void addEdge(int a, int b, double W)
    {
        ...
        if (it_a != Adj.end())
        {
            for (auto it_b = it_a->second.begin(); it_b != it_a->second.end(); it_b++)
                if (it_b->first == b)
                {
                    fl = false;
                    break;
...
{ // Ввод базовых параметров графа
    ...
    // Добавление ребра
    // Вес указывается третьей переменной.
    cout << endl << "Enter new edge: " << endl;
    int a, b;
    double w;
    cin >> a >> b >> w;
    gr.addEdge(a, b, w);
