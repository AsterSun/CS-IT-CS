...
void printOnlyLeafs(tree *tr) {		// Функция аналогична прямому обходу, но есть
	if (tr) {						// дополнительное условие - чтобы у узла не было
		if (!tr->left && !tr->right)// детей. Если детей нет - выводим, иначе - идём дальше.
			cout << tr->inf << " ";
		printOnlyLeafs(tr->left);
		printOnlyLeafs(tr->right);
	}
}
...
int main() {
	...
	print(tr, k);
	cout << "Only leafs:" << endl; // Выводим листья
	printOnlyLeafs(tr);
	cout << endl;
	return 0;
}