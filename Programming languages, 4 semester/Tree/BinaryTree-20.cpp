...
// Функция удаления. Удаление будем совершать рекурсивно симметричным обходом
// Функция аналогична симметричному обходу для вывода, но вместо функции cout
// используется функция Delete.
// Так как сначала идёт удаление элемента и только потом переход к правой ветке,
// то элемент правой ветки нужно сохранить. Левый элемент сохранять не обязательно,
// это сделано для однотипности кода. Для функции Delete необходимо значение указателя
// на удаляемый элемент и корень дерева, поэтому в данную функцию необходимо передавать
// оба этих значение. tr - корень, p - указатель.
void inorderDelIfEven(tree *&tr, tree *p) {
	if (p) {
		tree *bufL = p->left;
		tree *bufR = p->right;
		inorderDelIfEven(tr, bufL);
		if (!(p->inf % 2))
			Delete(tr, p);
		inorderDelIfEven(tr, bufR);
	}
}
...
int main() {
	...
	inorderDelIfEven(tr, tr); // Запускаем функцию удаления чётных элементов
	cout << endl;
	inorder(tr); // Выводим модифицированное дерево
	return 0;
}