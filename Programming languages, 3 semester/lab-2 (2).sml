...
(* Задание 1. getNth *)
fun getNth (x :: _, 0) = x
  | getNth (xs :: xl, n) = getNth (xl, n-1)
  | getNth (_) = raise List.Empty

(* Задание 2. reverseAppend *)
fun reverseAppend ([], reverseLst) = reverseLst
  | reverseAppend (xs :: xl, reverseLst) =
    reverseAppend (xl, xs :: reverseLst)
...
(* Задание 6. sumCards *)
fun sumCards (cardsumm) =
  let
    fun accumFun ([], accum) = accum
      | accumFun (cardsumms :: cardsumml, accum) =
        accumFun (cardsumml, cardValue (cardsumms) + accum)
  in
    accumFun (cardsumm, 0)
  end
...
fun removeCard (cs, c, e) = 
  let
    fun revomeNthAccumFun (_, []) = raise e 
      | revomeNthAccumFun (x, xs :: xl) =
        if isSameCard (xs, c) then reverseAppend (x, xl)
          else revomeNthAccumFun (xs :: x, xl) 
  in
    revomeNthAccumFun ([], cs)
  end

(* Задание 9. insertElem *)
fun insertElem (cs, c, n) =
  let
    fun insertElemFun (cs', c' :: cs, n) =
          insertElemFun (c' :: cs', cs, n - 1)
      | insertElemIter (cs', cs, 0) =
          reverseAppend (cs', c :: cs)
      | insertElemFun _ = raise List.Empty
  in
    insertElemFun ([], cs, n)
  end
